...
def calib_camera(model3D, fidu_XY):
    ...
    imagePoints = np.ascontiguousarray(fidu_XY[17:,:]).reshape((-1,1,2))
    #ret, rvecs, tvec = cv2.solvePnP(model3D.model_TD[17:,:], imagePoints, model3D.out_A, model3D.distCoeff, None, None, False,flags=cv2.SOLVEPNP_EPNP)
    #ret, rvecs, tvec = cv2.solvePnP(model3D.model_TD[17:,:], imagePoints, model3D.out_A,  model3D.distCoeff, rvecs, tvec, True,flags=cv2.SOLVEPNP_ITERATIVE)

    ret, rvecs, tvec = cv2.solvePnP(model3D.model_TD[17:,:], imagePoints, model3D.out_A, model3D.distCoeff, None, None, False)
